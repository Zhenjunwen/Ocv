...
class OvcAutoMake:
    def __init__(self, preloaddata):
        ...
        self.filename = "log/m1-%s-%s-%s.txt" % (self.launchtime, self.lastlong, self.picnum)
    ...
    def save(self):
        print('保存')
        self.savename = "%s-%s-m1-%s-%s" %(self.totaltime, self.starttime.strftime("%H:%M:%S"), self.lastlong, self.picnum)
        print(self.savename)
